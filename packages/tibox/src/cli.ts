...
cli
  ...
  .action(async (root: string, options: DevOptions & GlobalCLIOptions) => {
    ...
    try {
      ...
      createLogger(options.logLevel).error(chalk.red(`error during build:\n${e.stack}`))
      process.exit(1)
    }
  })

cli.command('build [root]', '构建小程序').action(async (root: string, options: BuildOptions & GlobalCLIOptions) => {
  createLogger(options.logLevel).warn(chalk.green(`root:${root}, options:${JSON.stringify(options, null, 2)}`))
