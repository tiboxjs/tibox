...
import type { Context, Task } from './tasks/task'
import {
  // CLIENT_ENTRY,
  // CLIENT_PUBLIC_PATH,
  // ENV_PUBLIC_PATH,
...
export async function parseDir(
  ...
  if ((!ignore || !ignore.test(pathDir)) && (await isFileExist(pathDir))) {
    const statRes = await fsp.stat(pathDir)
    if (statRes.isFile()) {
      return [path.resolve(pathDir)]
    } else {
      const readDirResult = await fsp.readdir(pathDir)
      if (recursive) {
        let result: string[] = []
...
export async function traceOutUnused(context: Context, wholeTask: Record<string, Task>): Promise<void> {
  ...
  // logger.info(colors.blueBright(`allFiles: ${parseResult.length}`));
  const unTrackedFiles = _.pull(parseResult, ..._.map(wholeTask, task => task.relativeToRootPath))

  // const trackedFiles = taskManager.files();
  // logger.info(
  //   colors.blueBright(`trackedFiles: ${JSON.stringify(trackedFiles, null, 2)}`)
  // );
  ...
  if (unTrackedFiles.length) {
    ...
    _.forEach(unTrackedFiles, (item, index) => {
      logger.info(colors.yellow(`  ${index + 1}. ${item}`))
    })
  }
}